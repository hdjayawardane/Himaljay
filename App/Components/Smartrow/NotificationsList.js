      ...
      <View style={[style.NotificationWraper]}>
        <View style={[style.listRow,{backgroundColor:css.dynamic.rows.backgroundColor,borderRadius:css.dynamic.general.rounded+""=="true"?10:0}]}>
           <View style={{height:50,width:50,alignItems:"center"}}>
              {/*  Icon  */}
              <MaterialIcons name={this.props.isRead?"notifications-none":"notifications"} size={30} style={[{marginTop:10},{color: "#219bca"}]} />
           </View>
           <View style={{flex:1,padding:10,paddingLeft:0}}>
           <Text numberOfLines={1} style={[style.titleinNotification,rtlText,{color: "#219bca"}]}>{this.state.title}</Text>
           <Text numberOfLines={3} style={[style.descriptionInNotification,rtlText,{color: "#219bca"}]}>{this.state.description}</Text>
           </View>
        </View>
      </View>
    );
