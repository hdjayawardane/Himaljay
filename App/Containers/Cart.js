...
import {View,TouchableOpacity,StyleSheet,ScrollView,AsyncStorage,FlatList,ActivityIndicator,Image, UIManager, LayoutAnimation} from "react-native";
...
export default class Cart extends Component {
  ...
  */
  updateCartContent(newCart,animating1){
    ...
    console.log("The state of animating is " + animating1);
  }
  ...
  renderItem(data){
    ...
    var listingSetup={
      "fields": {
        ...
        "image": "image",
        "subtitle": "price",
        "subtitleFunctions": "multiply~"+item.qty+",roundOn,toCurrency~USD",
        "title": "name"
      },
      "listing_style": "list"
    };
       ...
       return(
        <PayPalPayment
           ...
           setPaymentStatus={(payment_status)=>{
             console.log(T.status+payment_status);
  ...
  */
  showEmptyState(numItems){
    console.log("Number of items is " + numItems);
  ...
  render() {
    ...
    return (
        ...
        <ConditionalDisplay condition={!this.props.data.hideTopImage}>
          ...
          <ScrollableTabView
            locked={true}
            style={[css.layout.scrollableTabViewStyle]}
            initialPage={0}
              ...
              <View style={{flex:1}}>
                ...
                <View style={{height:60}}>
                   <ButtonUNI
                    ...
                    onPress={this.state.cart&&this.state.cart.length>0?()=>{this.setState({currentPosition:1})}:null}
                    text={T.next}
                  />
                </View>
                </View>
